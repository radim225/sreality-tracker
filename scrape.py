...
snapshot, and regenerates a mobile-friendly dashboard."""
import json
import re
import statistics
import sys
    ...
    for tx_type in TRANSACTION_TYPES:
        ...
        while True:
            ...
            next_data, status = fetch_next_data(
                url, params={"region": SEARCH_REGION_TEXT, "page": page}
            )
            if next_data is None:
                break
            ...
            for r in page_results:
                ...
                if sub.get("value") not in DISPOSITION_CODES:
                    continue
                results.append(parse_comparable(r, tx_type))
...
def parse_comparable(r, tx_type):
    ...
    slug = urllib.parse.quote((r.get("categorySubCb") or {}).get("name", "x"))
    detail_url = (
...
<div class="card">
  ...
  <div class="seed-grid">
    ...
    <div><b>Rent</b>{fmt_czk(seed.get('rent_czk'))}</div>
    <div><b>Fees</b>{fmt_czk(seed.get('fees_czk'))}</div>
    ...
    <div><b>Floor</b>{seed.get('floor_number')}/{seed.get('floors_total')}</div>
    <div><b>Locality</b>{seed.get('locality') or '—'}</div>
    <div><b>Seller</b>{seed.get('seller_name') or '—'}</div>
    <div><b>Photos</b>{seed.get('photo_count') if seed.get('photo_count') is not None else '—'}</div>
    <div><b>Deposit</b>{fmt_czk(seed.get('refundable_deposit_czk'))}</div>
  </div>
  <p><a href="{seed.get('url')}" target="_blank">Open on Sreality.cz →</a></p>
</div>
...
<footer>{len(comparables)} comparable listings tracked · generated by scrape.py</footer>

<script>
const DATA = {data_json};
const CHANGED_IDS = new Set({changed_ids_json});
...
function render() {{
  ...
  tbody.innerHTML = rows.map(r => `
    <tr class="${{CHANGED_IDS.has(r.id) ? 'changed' : ''}}">
...
def main():
    ...
    print(f"Found {len(comparables)} comparable listings", file=sys.stderr)
